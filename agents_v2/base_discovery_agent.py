...
# Import Entity for type hints (domain model integration)
if TYPE_CHECKING:
...
class BaseDiscoveryAgent(ABC):
    ...
    def discover(
        ...
        try:
            ...
            # =================================================================
            # DOMAIN MODEL INTEGRATION (Phase 2) - Feature flagged
            # =================================================================
            try:
                from config_v2 import ENABLE_DOMAIN_MODEL
                if ENABLE_DOMAIN_MODEL:
                    self._integrate_domain_model(entity=entity)
            except Exception as e:
                self.logger.warning(f"Domain model integration failed: {e}")
                # Don't fail discovery if domain model integration fails
            # =================================================================
    ...
    def _integrate_domain_model(self, entity: str):
        """
        Integrate domain models after discovery completes (Phase 2).

        This method is called when ENABLE_DOMAIN_MODEL=true and converts
        FactStore facts into domain model aggregates, then syncs back to
        InventoryStore for backward compatibility.
        ...
        # Convert entity string to enum
        entity_enum = Entity.TARGET if entity.lower() == "target" else Entity.BUYER

        # Only integrate for domains that have domain models
        if self.domain == "applications":
            self._integrate_applications(entity_enum)
        elif self.domain == "infrastructure":
            self._integrate_infrastructure(entity_enum)
        elif self.domain == "organization":
            self._integrate_organization(entity_enum)
        else:
            self.logger.info(f"Domain model not available for {self.domain}, skipping integration")

    def _integrate_applications(self, entity: 'Entity'):
        """Integrate Application domain model."""
        ...
        self.logger.info(f"[DOMAIN MODEL] Integrating applications (entity={entity.value})")

        # Convert FactStore → Application domain models
        adapter = FactStoreAdapter()
        repo = ApplicationRepository()
        applications = adapter.load_applications(self.fact_store, repo, entity_filter=entity)

        self.logger.info(f"[DOMAIN MODEL] Created {len(applications)} application aggregates")

        # Sync back to InventoryStore (dual write for backward compatibility)
        if self.inventory_store:
            inv_adapter = InventoryAdapter()
            inv_adapter.sync_applications(applications, self.inventory_store)
            self.logger.info(f"[DOMAIN MODEL] Synced {len(applications)} applications to InventoryStore")

        print(f"[DOMAIN MODEL] Applications: {len(applications)} aggregates created")

    def _integrate_infrastructure(self, entity: 'Entity'):
        """Integrate Infrastructure domain model."""
        from domain.adapters.fact_store_adapter import FactStoreAdapter
        from domain.infrastructure.repository import InfrastructureRepository
        from domain.adapters.inventory_adapter import InventoryAdapter

        self.logger.info(f"[DOMAIN MODEL] Integrating infrastructure (entity={entity.value})")

        # Convert FactStore → Infrastructure domain models
        adapter = FactStoreAdapter()
        repo = InfrastructureRepository()
        infrastructures = adapter.load_infrastructure(self.fact_store, repo, entity_filter=entity)

        self.logger.info(f"[DOMAIN MODEL] Created {len(infrastructures)} infrastructure aggregates")

        # Sync back to InventoryStore (dual write for backward compatibility)
        if self.inventory_store:
            inv_adapter = InventoryAdapter()
            inv_adapter.sync_infrastructure(infrastructures, self.inventory_store)
            self.logger.info(f"[DOMAIN MODEL] Synced {len(infrastructures)} infrastructure to InventoryStore")

        print(f"[DOMAIN MODEL] Infrastructure: {len(infrastructures)} aggregates created")

    def _integrate_organization(self, entity: 'Entity'):
        """Integrate Organization (Person) domain model."""
        from domain.adapters.fact_store_adapter import FactStoreAdapter
        from domain.organization.repository import PersonRepository
        from domain.adapters.inventory_adapter import InventoryAdapter

        self.logger.info(f"[DOMAIN MODEL] Integrating organization (entity={entity.value})")

        # Convert FactStore → Person domain models
        adapter = FactStoreAdapter()
        repo = PersonRepository()
        people = adapter.load_people(self.fact_store, repo, entity_filter=entity)

        self.logger.info(f"[DOMAIN MODEL] Created {len(people)} person aggregates")

        # Sync back to InventoryStore (dual write for backward compatibility)
        if self.inventory_store:
            inv_adapter = InventoryAdapter()
            inv_adapter.sync_people(people, self.inventory_store)
            self.logger.info(f"[DOMAIN MODEL] Synced {len(people)} people to InventoryStore")

        print(f"[DOMAIN MODEL] Organization: {len(people)} aggregates created")
